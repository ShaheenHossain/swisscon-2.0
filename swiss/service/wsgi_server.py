...
# User code must use the exceptions defined in ``swiss.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, swiss.exceptions.RedirectWarning):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, swiss.exceptions.AccessError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, swiss.exceptions.AccessDenied):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, swiss.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = swiss.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, swiss.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(swiss.tools.exception_to_unicode(e), formatted_info)
...
def application_unproxied(environ, start_response):
    ...
    # swiss.service.web_services.objects_proxy.dispatch().
    ...
    with swiss.api.Environment.manage():
        result = swiss.http.root(environ, start_response)
