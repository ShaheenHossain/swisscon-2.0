...
import swiss
from swiss import SUPERUSER_ID
from swiss.exceptions import AccessDenied
import swiss.release
import swiss.sql_db
import swiss.tools
from swiss.sql_db import db_connect
from swiss.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not swiss.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and swiss.tools.config.verify_admin_password(passwd):
        return True
    raise swiss.exceptions.AccessDenied()

# This should be moved to swiss.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = swiss.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            swiss.modules.db.initialize(cr)
            swiss.tools.config['load_language'] = lang
            cr.commit()

        registry = swiss.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = swiss.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = swiss.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = swiss.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = swiss.tools.config['db_template']
    ...
    if swiss.tools.config['unaccent']:
        try:
            db = swiss.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    swiss.sql_db.close_db(db_original_name)
    db = swiss.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = swiss.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = swiss.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = swiss.tools.config.filestore(db_original_name)
    to_fs = swiss.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    swiss.modules.registry.Registry.delete(db_name)
    swiss.sql_db.close_db(db_name)
    ...
    fs = swiss.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'swiss_dump': '1',
        'db_name': cr.dbname,
        'version': swiss.release.version,
        'version_info': swiss.release.version_info,
        'major_version': swiss.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = swiss.tools.config.filestore(db_name)
            ...
            swiss.tools.exec_pg_command(*cmd)
            if stream:
                swiss.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                swiss.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = swiss.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if swiss.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = swiss.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = swiss.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    swiss.modules.registry.Registry.delete(old_name)
    swiss.sql_db.close_db(old_name)
    ...
    old_fs = swiss.tools.config.filestore(old_name)
    new_fs = swiss.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    swiss.tools.config.set_admin_password(new_password)
    swiss.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        swiss.tools.config['update']['base'] = True
        swiss.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@swiss.tools.mute_logger('swiss.sql_db')
...
def list_dbs(force=False):
    if not swiss.tools.config['list_db'] and not force:
        raise swiss.exceptions.AccessDenied()

    if not swiss.tools.config['dbfilter'] and swiss.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, SwissCRM will not
        ...
        res = sorted(db.strip() for db in swiss.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [swiss.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of SwissCRM
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if swiss.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        swiss.sql_db.close_db(database_name)
...
def exp_list_lang():
    return swiss.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(swiss.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
