...
from swiss.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = swiss.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(swiss.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if swiss.registry(dbname)._init and not swiss.tools.config['test_enable']:
                    raise swiss.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    swiss.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = swiss.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = swiss.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with swiss.registry(db).cursor() as cr:
