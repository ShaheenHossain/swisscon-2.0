...
# *broken*
NO_POSTMORTEM = (swiss.exceptions.AccessDenied,
                 swiss.exceptions.UserError,
                 swiss.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                swiss.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = swiss.service.common.dispatch
        elif service_name == 'db':
            dispatch = swiss.service.db.dispatch
        elif service_name == 'object':
            dispatch = swiss.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                swiss.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                swiss.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(swiss.tools.exception_to_unicode(e))
        swiss.tools.debugger.post_mortem(swiss.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all SwissCRM & ERPWeb request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in swiss.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~swiss.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~swiss.api.Environment` bound to current request. """
        if self._env is None:
            self._env = swiss.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            swiss.tools.debugger.post_mortem(
                swiss.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        SwissCRM & ERPimplements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <swiss.http.WebRequest.csrf_token`, the
          :data:`~swiss.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          SwissCRM) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (swiss.exceptions.UserError,
            ...
            error = {
                'code': 200,
                'message': "SwissCRM & ERPServer Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "SwissCRM & ERPSession Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "SwissCRM & ERPSession Expired"
...
HTTP methods). See
https://doc.swisshq.com/2.0/developer/reference/http.html#csrf for
more details.
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["swiss", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('swiss.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = swiss.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = swiss.api.Environment(request.cr, self.uid, self.context)
...
SWISS_DISABLE_SESSION_GC = str2bool(os.environ.get('SWISS_DISABLE_SESSION_GC', '0'))

if SWISS_DISABLE_SESSION_GC:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or swiss.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = swiss.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in swiss.http._generate_routing_rules([''] + swiss.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in swiss.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        swiss.registry(db).check_signaling()
                        with swiss.tools.mute_logger('swiss.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = swiss.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if swiss.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = swiss.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif swiss.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, SwissCRM will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in swiss.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = swiss.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = swiss.tools.ustr(filename)
