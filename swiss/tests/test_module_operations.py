...
import swiss
from swiss.tools import topological_sort, unique
from swiss.netsvc import init_logger
from swiss.tests import standalone_tests
import swiss.tests.loader

_logger = logging.getLogger('swiss.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with swiss.api.Environment.manage():
        with swiss.registry(db_name).cursor() as cr:
            env = swiss.api.Environment(cr, swiss.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of SwissCRM modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str,
        help="Comma-separated list of paths to directories containing extra SwissCRM modules")
...
def test_full(args):
    ...
    with swiss.api.Environment.manage():
        with swiss.registry(args.database).cursor() as cr:
            env = swiss.api.Environment(cr, swiss.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = swiss.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        swiss.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        swiss.tools.config['addons_path'] = ','.join([args.addons_path, swiss.tools.config['addons_path']])
        if args.data_dir:
            swiss.tools.config['data_dir'] = args.data_dir
        swiss.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('swiss.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('swiss.sql_db').setLevel(logging.CRITICAL)
