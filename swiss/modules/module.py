...
import swiss
import swiss.tools as tools
import swiss.release as release
...
# ad_paths is a deprecated alias, please use swiss.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"swiss.modules.module.ad_paths" is a deprecated proxy to '
        '"swiss.addons.__path__".', DeprecationWarning, stacklevel=2)
    return swiss.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "swiss.addons".',
    ...
    def load_module(self, name):
        ...
        swiss_name = re.sub(r'^openerp.addons.(\w+)$', r'swiss.addons.\g<1>', name)

        swiss_module = sys.modules.get(swiss_name)
        if not swiss_module:
            swiss_module = importlib.import_module(swiss_name)

        sys.modules[name] = swiss_module

        return swiss_module

class SwissCRMHook(object):
    """ Makes swiss package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to swiss.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'swiss\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `swiss.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^swiss.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("swiss.addons.base.maintenance.migrations", "swiss.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``swiss.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook swiss.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in swiss.addons.__path__:
        swiss.addons.__path__.append(dd)

    # hook swiss.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in swiss.addons.__path__:
            swiss.addons.__path__.append(ad)

    # hook swiss.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in swiss.addons.__path__ and os.path.isdir(base_path):
        swiss.addons.__path__.append(base_path)

    # hook swiss.upgrade on upgrade-path
    from swiss import upgrade
    ...
    # create decrecated module alias from swiss.addons.base.maintenance.migrations to swiss.upgrade
    spec = importlib.machinery.ModuleSpec("swiss.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["swiss.addons.base.maintenance"] = maintenance_pkg
    sys.modules["swiss.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to swiss and "crm"-like to swiss.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, SwissCRMHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in swiss.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = swiss.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in swiss.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('swiss.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['swiss.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in swiss.addons.__path__:
