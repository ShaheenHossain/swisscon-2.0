#swiss.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.swissrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store SwissCRM data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'swiss_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "swiss.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="swiss.http.rpc.request:DEBUG", help='shortcut for --log-handler=swiss.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="swiss.http.rpc.response:DEBUG", help='shortcut for --log-handler=swiss.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="swiss.http:DEBUG", help='shortcut for --log-handler=swiss.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="swiss.sql_db:DEBUG", help='shortcut for --log-handler=swiss.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate SwissCRM to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes swiss.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            swiss.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        swiss.netsvc.init_logger()
        self._warn_deprecated_options()
        swiss.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'swiss.conf')
        else:
            rcfilepath = os.path.expanduser('~/.swissrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.swissrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.swissrc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('SWISS_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from swiss.modules.module import MANIFEST_NAMES
