...
from swiss.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if swiss.tools.config['db_host']:
        env['PGHOST'] = swiss.tools.config['db_host']
    if swiss.tools.config['db_port']:
        env['PGPORT'] = str(swiss.tools.config['db_port'])
    if swiss.tools.config['db_user']:
        env['PGUSER'] = swiss.tools.config['db_user']
    if swiss.tools.config['db_password']:
        env['PGPASSWORD'] = swiss.tools.config['db_password']
    return env
...
def _fileopen(path, mode, basedir, pathinfo, basename=None, filter_ext=None):
    ...
    paths = swiss.addons.__path__ + [config['root_path']]
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = swiss.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('swiss.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('swiss.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = swiss.fields.Datetime.from_string(value)
            value = swiss.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = swiss.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = swiss.fields.Datetime.from_string(value)
    else:
        timestamp = value
