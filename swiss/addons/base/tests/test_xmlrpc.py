...
from swiss.exceptions import AccessDenied, AccessError
from swiss.http import _request_stack

import swiss.tools
from swiss.tests import common
from swiss.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, swiss.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
