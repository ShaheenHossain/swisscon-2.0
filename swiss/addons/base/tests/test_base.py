...
from swiss.tests.common import TransactionCase, BaseCase
from swiss.tools import mute_logger
from swiss.tools.safe_eval import safe_eval, const_eval, expr_eval
...
class TestSafeEval(BaseCase):
    ...
    @mute_logger('swiss.tools.safe_eval')
    def test_05_safe_eval_forbiddon(self):
        ...
        with self.assertRaises(ValueError):
            safe_eval("import swiss", mode="exec")
...
class TestBase(TransactionCase):
    ...
    def test_70_archive_internal_partners(self):
        ...
        test_user = self.env['res.users'].create({
                                'login': 'test@swissconsultings.ch',
