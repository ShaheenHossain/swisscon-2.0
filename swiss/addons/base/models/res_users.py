...
from swiss import api, fields, models, tools, SUPERUSER_ID, _
from swiss.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from swiss.exceptions import AccessDenied, AccessError, UserError, ValidationError
from swiss.http import request
from swiss.modules.module import get_module_resource
from swiss.osv import expression
from swiss.service.db import check_super
from swiss.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    ...
    def _get_default_image(self):
        """ Get a default image when the user is created without image
            ...
            https://github.com/swiss/swiss/blob/11.0/swiss/addons/base/res/res_partner.py
        """
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by SwissCRM (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: swiss.exceptions.AccessDenied when old password is wrong
        :raise: swiss.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "https://swissconsultings.ch/administration/deployment/deploy.html#https for details.",
                    source
                )
