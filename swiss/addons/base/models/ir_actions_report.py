...
from swiss.exceptions import UserError, AccessError
from swiss.tools.safe_eval import safe_eval, time
from swiss.tools.misc import find_in_path
from swiss.tools import config
from swiss.sql_db import TestCursor
from swiss.http import request
from swiss.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of SwissCRM so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at SwissCRM start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start SwissCRM with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "SwissCRM is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"
