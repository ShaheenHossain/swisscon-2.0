...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('swiss.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='swiss')
...
DEFAULT_LOG_CONFIGURATION = [
    'swiss.http.rpc.request:INFO',
    'swiss.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['swiss:DEBUG', 'swiss.sql_db:INFO', 'swiss.http.rpc:DEBUG'],
    'debug_rpc': ['swiss:DEBUG', 'swiss.sql_db:INFO', 'swiss.http.rpc.request:DEBUG'],
    'debug': ['swiss:DEBUG', 'swiss.sql_db:INFO'],
    'debug_sql': ['swiss.sql_db:DEBUG'],
    'info': [],
    'runbot': ['swiss:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['swiss:WARNING', 'werkzeug:WARNING'],
    'error': ['swiss:ERROR', 'werkzeug:ERROR'],
    'critical': ['swiss:CRITICAL', 'werkzeug:CRITICAL'],
}
