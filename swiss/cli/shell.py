...
class Shell(Command):
    """Start swiss in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        swiss.cli.server.report_configuration()
        swiss.service.server.start(preload=[], stop=True)
        ...
        with swiss.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = swiss.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = swiss.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
