...
__author__ = swiss.release.author
__version__ = swiss.release.version

# Also use the `swiss` logger for the main script.
_logger = logging.getLogger('swiss')
...
def report_configuration():
    ...
    """
    config = swiss.tools.config
    _logger.info("SwissCRM version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', swiss.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = swiss.tools.config
    if not swiss.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = swiss.modules.registry.Registry.new(dbname)
        with swiss.api.Environment.manage():
            with registry.cursor() as cr:
                swiss.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with swiss.api.Environment.manage():
        with registry.cursor() as cr:
            swiss.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    swiss.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                swiss.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except swiss.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        swiss.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = swiss.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the swiss server (default command)"""
    def run(self, args):
        main(args)
