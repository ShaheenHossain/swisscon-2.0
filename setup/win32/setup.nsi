...
!ifndef SERVICENAME
	!define SERVICENAME 'swiss-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\swissbuild'
!endif

!define PRODUCT_NAME "SwissCRM"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\SwissCRM Server ${VERSION}"
...
OutFile "swiss_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\swiss-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\swiss-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\swiss-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\swiss-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://swissconsultings.ch/contactus"
...
; English
LangString DESC_SwissCRM_Server ${LANG_ENGLISH} "Install the SwissCRM Server with all the SwissCRM standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by SwissCRM."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact SwissCRM for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the SwissCRM Server"
...
LangString TITLE_SwissCRM_Server ${LANG_ENGLISH} "SwissCRM Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start SwissCRM"

; French
LangString DESC_SwissCRM_Server ${LANG_FRENCH} "Installation du Serveur SwissCRM avec tous les modules Odoo standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par SwissCRM."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez SwissCRM pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur SwissCRM"
...
LangString TITLE_SwissCRM_Server ${LANG_FRENCH} "Serveur SwissCRM"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer SwissCRM"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_SwissCRM_Server) SectionSwissCRM_Server
    SectionIn 1 2
    ...
    # If there is a previous install of the SwissCRM Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\swiss.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\swiss.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\swiss.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\swiss.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\swiss.conf" "options" "addons_path" "$INSTDIR\server\swiss\addons"
    WriteIniStr "$INSTDIR\server\swiss.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\swiss.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\swiss-bin" --stop-after-init --logfile "$INSTDIR\server\swiss.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\swiss-bin\""'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_SwissCRM" \
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@swissconsultings.ch"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.swissconsultings.ch"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@swissconsultings.ch"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionSwissCRM_Server} $(DESC_SwissCRM_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        ${If} ${RunningX64}
            StrCpy $INSTDIR "$PROGRAMFILES64\SwissCRM ${VERSION}"
        ${Else}
            StrCpy $INSTDIR "$PROGRAMFILES\SwissCRM ${VERSION}"
        ${EndIf}
...
Function ShowPostgreSQL
    SectionGetFlags ${SectionSwissCRM_Server} $0
