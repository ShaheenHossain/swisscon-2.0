...
exec(open(os.path.join(ROOTDIR, 'swiss', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/swiss && \
    groupadd -g %(group_id)s swiss && \
    useradd -u %(user_id)s -g swiss swiss -d /var/lib/swiss && \
    mkdir /data && \
    chown swiss:swiss /var/lib/swiss /data
USER swiss
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class SwissCRMTestTimeoutError(Exception):
    pass


class SwissCRMTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=7073, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise SwissCRMTestError("Installation of package failed")
        else:
        ...
        raise SwissCRMTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/swiss_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.swiss_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'swiss/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'swiss-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='swiss', exposed_port=None, detach=False, timeout=None):
    ...
    def test_swiss(self):
        logging.info('Starting to test SwissCRM & ERPinstall test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'swiss.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise SwissCRMTestTimeoutError('SwissCRM & ERPpid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise SwissCRMTestError('Error while installing/starting SwissCRM & ERPafter %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'swiss-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/swiss-*.tar.gz' % self.args.build_dir)[0], '%s/swiss_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/swiss-*.zip' % self.args.build_dir)[0], '%s/swiss_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/swiss_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s swiss"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su swiss -s /bin/bash -c "swiss -d mycompany -i base --stop-after-init"',
            'su swiss -s /bin/bash -c "swiss -d mycompany --pidfile=/data/src/swiss.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'swiss-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=7073, timeout=300)
        self.test_swiss()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/swiss (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../swiss_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'swiss-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/swiss_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su swiss -s /bin/bash -c "swiss -d mycompany -i base --stop-after-init"',
            'su swiss -s /bin/bash -c "swiss -d mycompany --pidfile=/data/src/swiss.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'swiss-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=7073, timeout=300)
        self.test_swiss()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'swiss-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/swiss-*.noarch.rpm' % self.args.build_dir)[0], '%s/swiss_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/swiss_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su swiss -s /bin/bash -c "swiss -c /etc/swiss/swiss.conf -d mycompany -i base --stop-after-init"',
            'su swiss -s /bin/bash -c "swiss -c /etc/swiss/swiss.conf -d mycompany --pidfile=/data/src/swiss.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'swiss-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=7073, timeout=300)
        self.test_swiss()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'swiss-rpm-createrepo-%s' % TSTAMP)
...
class KVM(object):
    ...
    def start(self):
        kvm_cmd = [
            ...
            "-net", "user,hostfwd=tcp:127.0.0.1:10022-:22,hostfwd=tcp:127.0.0.1:17073-:7073,hostfwd=tcp:127.0.0.1:15432-:5432",
            "-m", "2048",
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/swissbuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing SwissCRM & ERPfiles to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/swiss_setup_*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('swiss_setup_')[1].split('.exe')[0]

        self.rsync(['%s' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"SwissCRM & ERP%s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"SwissCRM & ERP{sv}"/python/python.exe \'c:\\Program Files\\SwissCRM & ERP{sv}\\server\\swiss-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/swiss/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/swiss/vm/win1036/id_rsa', help="%(default)s")
