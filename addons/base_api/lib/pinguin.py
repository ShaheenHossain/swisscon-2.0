...
def transform_dictfields_to_list_of_tuples(record, dct, ENV=False):
    ...
    :param swiss.models.Model record: The model object.
    ...
    for key, value in dct.items():
        if isinstance(value, dict):
            model_obj = get_model_for_read(
                fields_with_meta[key]["relation"], ENV)
            inner_result = transform_dictfields_to_list_of_tuples(
                model_obj, value, ENV)
            is_2many = fields_with_meta[key]["type"].endswith("2many")
            result[key] = list(
                inner_result) if is_2many else tuple(inner_result)
        else:
            result[key] = value
...
def get_model_for_read(model, ENV=False):
    ...
    if not test_mode:
        ...
        # Contrary to ISOLATION_LEVEL_SERIALIZABLE as per SwissCRM Standard
...
def get_dict_from_record(
    ...
    :param swiss.models.Model record: The singleton record to load.
    ...
    _spec = [fld for fld in spec if fld not in exclude_fields] + \
        list(include_fields)
    ...
    for field in _spec:
        ...
        # Normal field, or unspecified relational
            ...
            if not hasattr(record, field):
                raise swiss.exceptions.ValidationError(
                    swiss._('The model "%s" has no such field: "%s".')
                    % (record._name, field)
                )
