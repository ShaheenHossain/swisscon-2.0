...
from swiss.http import Response
from swiss.modules.module import get_resource_path

from swiss.addons.hw_drivers.main import iot_devices
from swiss.addons.hw_drivers.tools import helpers
from swiss.addons.web.controllers import main as web
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('swiss.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/swiss/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('swiss-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_swiss_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'swiss-remote-server.conf'
    ...
    def list_handlers(self):
        ...
        return handler_list_template.render({
            'title': "SwissCRM's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_swiss_server_url()
        })
    ...
    def load_iot_handlers(self):
        ...
        subprocess.check_call(["sudo", "service", "swiss", "restart"])
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "SwissCRM's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('swiss-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('swiss-enterprise-code.conf'),
        })
    ...
    def clear_credential(self):
        helpers.unlink_file('swiss-db-uuid.conf')
        helpers.unlink_file('swiss-enterprise-code.conf')
        subprocess.check_call(["sudo", "service", "swiss", "restart"])
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":7073'>"
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to SwissCRM Server'
            }
        else:
            res_payload['server'] = {
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('swiss-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":7073'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_swiss_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_swiss_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> SwissCRM server configuration',
            'breadcrumb': 'Configure SwissCRM Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_swiss_server_url() or 'Not configured yet',
    ...
    def remote_connect(self):
        ...
        2 - copy past this auth_token on the IoT Box : http://IoT_Box:7073/remote_connect
    ...
    def add_six_payment_terminal(self, terminal_id):
        helpers.write_file('swiss-six-payment-terminal.conf', terminal_id)
        subprocess.check_call(["sudo", "service", "swiss", "restart"])
        return 'http://' + helpers.get_ip() + ':7073'

    @http.route('/six_payment_terminal_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_six_payment_terminal(self):
        helpers.unlink_file('swiss-six-payment-terminal.conf')
        subprocess.check_call(["sudo", "service", "swiss", "restart"])
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":7073'>"

    @http.route('/hw_proxy/upgrade', type='http', auth='none', )
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/swiss/", "--git-dir=/home/pi/swiss/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
        ...
        return upgrade_page_template.render({
            'title': "SwissCRM's IoTBox - Software Upgrade",
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/swiss/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/swiss/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/swiss/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/swiss/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios']).decode().split('\n')[-2]
