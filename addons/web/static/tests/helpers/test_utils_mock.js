swiss.define('web.test_utils_mock', function (require) {
"use strict";
...
function _mockGlobalObjects(params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            swiss.debug = params.config.debug;
        }
    }
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any SwissCRM widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in SwissCRMAfterTestHook?
 ...
 *   used to generate mock answers for every kind of routes supported by swiss
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught SwissCRM
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other SwissCRM events bubbling up are intercepted
