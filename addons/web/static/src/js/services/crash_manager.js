swiss.define('web.ErrorDialogRegistry', function (require) {
"use strict";
...
swiss.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    init: function () {
        ...
        this.swissExceptionTitleMap = {
            'swiss.addons.base.models.ir_mail_server.MailDeliveryException': _lt("MailDeliveryException"),
            'swiss.exceptions.AccessDenied': _lt("Access Denied"),
            'swiss.exceptions.AccessError': _lt("Access Error"),
            'swiss.exceptions.MissingError': _lt("Missing Record"),
            'swiss.exceptions.UserError': _lt("User Error"),
            'swiss.exceptions.ValidationError': _lt("Validation Error"),
            'swiss.exceptions.Warning': _lt("Warning"),
        };
    ...
    rpc_error: function(error) {
        ...
        // SwissCRM custom exception: UserError, AccessError, ...
        if (_.has(this.swissExceptionTitleMap, error.data.name)) {
            error = _.extend({}, error, {
                data: _.extend({}, error.data, {
                    message: error.data.arguments[0],
                    title: this.swissExceptionTitleMap[error.data.name],
                }),
            });
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("SwissCRM Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("SwissCRM Warning"),
            buttons: [
...
core.crash_registry.add('swiss.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("SwissCRM Session Expired"),
                message: _t("Your SwissCRM session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
...
core.crash_registry.add('swiss.http.SessionExpiredException', session_expired);
