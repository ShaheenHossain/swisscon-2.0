...
(function () {
    ...
    swiss.testing = typeof QUnit === 'object';
    swiss.remainingJobs = jobs;
    swiss.__DEBUG__ = {
    ...
    swiss.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__swiss_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: swiss.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = swiss.__DEBUG__.getDependencies(job.name);
            ...
            missing = swiss.__DEBUG__.getMissingJobs();
            failed = swiss.__DEBUG__.getFailedJobs();
            ...
            if (swiss.debug || failed.length || unloaded.length) {
                ...
                if (swiss.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        swiss.__DEBUG__.jsModules = {
            missing: missing,
    ...
    swiss.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            swiss.processJobs(jobs, services);
