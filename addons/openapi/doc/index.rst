...
* `Install <https://swiss-development.readthedocs.io/en/latest/swiss/usage/install-module.html>`__ this module in a usual way
* Add ``openapi`` to `--load parameter <https://swiss-development.readthedocs.io/en/latest/admin/server_wide_modules.html>`__, e.g.::

    ./swiss-bin --workers=2 --load openapi,base,web --config=/path/to/swiss.conf
...
Note:
  The Swagger Editor sends requests directly from browser which leads to CORS error and work with it is not available in `swiss.sh`.
