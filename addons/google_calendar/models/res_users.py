...
import logging
import requests
from swiss.addons.google_calendar.models.google_sync import google_calendar_token
from datetime import timedelta


from swiss import api, fields, models, _
from swiss.exceptions import UserError
from swiss.loglevels import exception_to_unicode
from swiss.addons.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT
from swiss.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
...
class User(models.Model):
    ...
    google_calendar_cal_id = fields.Char('Calendar ID', copy=False, help='Last Calendar ID who has been synchronized. If it is changed, we remove all links between GoogleID and SwissCRM Google Internal ID')
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2swiss(recurrences)
        synced_events = self.env['calendar.event']._sync_google2swiss(events - recurrences, default_reminders=default_reminders)

        # SwissCRM -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_swiss2google(calendar_service)
        ...
        (events - synced_events)._sync_swiss2google(calendar_service)
