...
from swiss import api, fields, models, registry, _
from swiss.tools import ormcache_context
from swiss.exceptions import UserError
from swiss.osv import expression

from swiss.addons.google_calendar.utils.google_event import GoogleEvent
from swiss.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from swiss.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the SwissCRM database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in SwissCRM.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on SwissCRM DB (_sync_google2swiss).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_swiss2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2swiss(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in SwissCRM. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in SwissCRM
        :return: synchronized swiss recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        swiss_values = [
            dict(self._swiss_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_swiss = self.with_context(dont_notify=True)._create_from_google(new, swiss_values)
        ...
        if not self._context.get("dont_notify") and all(not e.is_recurrence() for e in google_events):
            new_swiss._notify_attendees()

        cancelled = existing.cancelled()
        cancelled_swiss = self.browse(cancelled.swiss_ids(self.env))
        cancelled_swiss._cancel()
        synced_records = (new_swiss + cancelled_swiss).with_context(dont_notify=self._context.get("dont_notify", False))
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and swiss server time are different
            updated = parse(gevent.updated)
            swiss_record = self.browse(gevent.swiss_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not swiss_record.write_date or updated >= pytz.utc.localize(swiss_record.write_date):
                vals = dict(self._swiss_values(gevent, default_reminders), need_sync=False)
                swiss_record._write_from_google(gevent, vals)
                synced_records |= swiss_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from SwissCRM to Google
    ...
    @api.model
    def _swiss_values(self, google_event: GoogleEvent, default_reminders=()):
    ...
    def _notify_attendees(self):
        ...
        This is called when creating new calendar events in _sync_google2swiss.
        At the initialization of a synced calendar, SwissCRM requests all events for a specific
