...
from swiss import api, fields, models, tools, _
...
class Meeting(models.Model):
    ...
    @api.model
    def _swiss_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._swiss_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._swiss_attendee_commands(google_event)
        values = {
    ...
    @api.model
    def _swiss_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(google_event.swiss_id(self.env)).attendee_ids
        ...
        for swiss_attendee in attendees_by_emails.values():
            ...
            if tools.email_normalize(swiss_attendee.email) not in emails:
                attendee_commands += [(2, swiss_attendee.id)]
                partner_commands += [(3, swiss_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _swiss_reminders_commands(self, reminders=()):
        commands = []
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_swiss_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user:
            ...
            # If several swiss users are attendees but the owner is not in swiss, the event will be duplicated on swiss database
            ...
            # values['extendedProperties']['private] should be used if the owner is not an swiss user
