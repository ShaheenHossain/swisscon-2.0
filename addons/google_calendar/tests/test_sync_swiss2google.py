...
from swiss.addons.google_calendar.models.res_users import User
from swiss.addons.google_calendar.models.google_sync import GoogleSync
from swiss.modules.registry import Registry
from swiss.addons.google_account.models.google_service import TIMEOUT
from swiss.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api


@patch.object(User, '_get_google_calendar_token', lambda user: 'dummy-token')
class TestSyncSwissCRM2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_swiss2google(self.google_service)
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'swissbot@example.com', 'self': True},
            'attendees': [{'email': 'jean-luc@opoo.com', 'responseStatus': 'needsAction'}],
            'extendedProperties': {'shared': {'%s_swiss_id' % self.env.cr.dbname: event.id}}
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_swiss2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15'},
            'end': {'date': '2020-01-16'},
            ...
            'extendedProperties': {'shared': {'%s_swiss_id' % self.env.cr.dbname: recurrence.id}}
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_swiss_id' % self.env.cr.dbname: event.recurrence_id.id}}
        }, timeout=3)
