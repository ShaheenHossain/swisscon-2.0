...
from swiss import api, exceptions, fields, models, _, SUPERUSER_ID
from swiss.tools import consteq, float_round, image_process, ustr
from swiss.exceptions import ValidationError
from swiss.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
...
class PaymentAcquirer(models.Model):
    ...
    capture_manually = fields.Boolean(string="Capture Amount Manually",
        help="Capture the amount from SwissCRM, when the delivery is completed.")
    ...
    module_to_buy = fields.Boolean(string='SwissCRM Enterprise Module', related='module_id.to_buy', readonly=True, store=False)

    image_128 = fields.Image("Image", max_width=128, max_height=128)

    payment_icon_ids = fields.Many2many('payment.icon', string='Supported Payment Icons')
    payment_flow = fields.Selection(selection=[('form', 'Redirection to the acquirer website'),
        ('s2s','Payment from SwissCRM')],
