...
import logging
import requests
from swiss.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from swiss.addons.microsoft_account.models.microsoft_service import MICROSOFT_TOKEN_ENDPOINT
from swiss.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> SwissCRM
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2swiss(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # SwissCRM -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_swiss2microsoft()
        ...
        (events - synced_events)._sync_swiss2microsoft()
