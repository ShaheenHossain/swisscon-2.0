...
from swiss.tests.common import HttpCase

from swiss.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a SwissCRM event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for SwissCRM events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to SwissCRM events
        # -----------------------------------------------------------------------------------------

        self.expected_swiss_event_from_outlook = {
            "name": "simple_event",
            "description": "my simple event",
            "active": True,
            "start": self.start_date,
        ...
        self.expected_swiss_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_swiss_recurrency_events_from_outlook = [
            {
    ...
    def assert_swiss_event(self, swiss_event, expected_values):
        """
        Assert that an SwissCRM event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        swiss_event_values = swiss_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(swiss_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, swiss_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_swiss_recurrence(self, swiss_recurrence, expected_values):
        """
        Assert that an SwissCRM recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        swiss_recurrence_values = swiss_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, swiss_recurrence_values.get(k), msg=f"'{k}' mismatch")
