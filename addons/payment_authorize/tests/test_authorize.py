...
from swiss.addons.payment_authorize.controllers.main import AuthorizeController
...
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('swiss.addons.payment_authorize.models.payment', 'ValidationError')
...
@swiss.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on swiss/swiss + create sandbox account
        authorize.write({
