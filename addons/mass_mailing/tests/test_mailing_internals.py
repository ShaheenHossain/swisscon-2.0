...
from swiss.addons.mass_mailing.tests.common import MassMailCommon
from swiss.tests.common import users, Form
from swiss.tools import formataddr, mute_logger
...
class TestMassMailFeatures(MassMailCommon):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
% set url = "www.swissconsultings.ch"
% set httpurl = "https://www.swiss.eu"
Website0: <a id="url0" href="https://www.swiss.tz/my/${object.name}">https://www.swiss.tz/my/${object.name}</a>
Website1: <a id="url1" href="https://www.swiss.be">https://www.swiss.be</a>
...
Email: <a id="url5" href="mailto:test@swissconsultings.ch">test@swissconsultings.ch</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'email',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.swiss.tz/my/%s' % contact.name, True),
                              ('url1', 'https://www.swiss.be', True),
                              ('url2', 'https://www.swissconsultings.ch', True),
                              ('url3', 'https://www.swiss.eu', True),
                              ('url4', 'https://www.example.com/foo/bar?baz=qux', True),
                              ('url5', 'mailto:test@swissconsultings.ch', False)]:
