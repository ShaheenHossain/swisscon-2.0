...
@tagged("swissbot")
class TestSwissCRMbot(TestMailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestSwissCRMbot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.swissbot = cls.env.ref("base.partner_root")
    ...
    def test_fetch_listener(self):
        channel = self.env['mail.channel'].with_user(self.user_employee).init_swissbot()
        partners = self.env['mail.channel'].channel_fetch_listeners(channel.uuid)
        swissbot = self.env.ref("base.partner_root")
        swissbot_in_fetch_listeners = [partner for partner in partners if partner['id'] == swissbot.id]
        self.assertEqual(len(swissbot_in_fetch_listeners), 1, 'swissbot should appear only once in channel_fetch_listeners')

    @mute_logger('swiss.addons.mail.models.mail_mail')
    def test_swissbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.swissbot_ping_body, 'partner_ids': [self.swissbot.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.swissbot,
                answer=False
            )
        # SwissCRMbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.swissbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last swissbot message instead
            sender=self.swissbot,
            answer=("@SwissCRMBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.swissbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.swissbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('swiss.addons.mail.models.mail_mail')
    def test_swissbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @swissbot right now", 'partner_ids': []})
        self.assertNextMessage(
