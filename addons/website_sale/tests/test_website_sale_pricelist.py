...
from swiss.addons.base.tests.common import TransactionCaseWithUserDemo, HttpCaseWithUserPortal
from swiss.addons.website.tools import MockRequest
from swiss.tests import tagged
from swiss.tests.common import HttpCase, TransactionCase
from swiss.tools import DotDict
...
class TestWebsitePriceList(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('swiss.addons.website_sale.models.website.Website.get_pricelist_available', wraps=self._get_pricelist_available)
        patcher.start()
...
def simulate_frontend_context(self, website_id=1):
    ...
    patcher = patch('swiss.addons.website.models.ir_http.get_request_website', wraps=get_request_website)
    patcher.start()
    self.addCleanup(patcher.stop)
