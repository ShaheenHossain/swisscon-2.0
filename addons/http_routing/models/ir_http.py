...
import swiss
from swiss import api, models, registry, exceptions, tools, http
from swiss.addons.base.models.ir_http import RequestUID, ModelConverter
from swiss.addons.base.models.qweb import QWebException
from swiss.http import request
from swiss.osv import expression
from swiss.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if swiss._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            swiss._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if swiss._geoip_resolver and request.httprequest.remote_addr:
                record = swiss._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except swiss.exceptions.MissingError:
