...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to SwissCRM database
        """
        server = helpers.get_swiss_server_url()
        if server:
            subject = helpers.read_file_first_line('swiss-subject.conf')
            if subject:
            ...
            _logger.warning('SwissCRM server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the swiss server with the updates
        """
