swiss.define('mail/static/src/models/messaging_notification_handler/messaging_notification_handler.js', function (require) {
'use strict';
...
function factory(dependencies) {
    ...
    class MessagingNotificationHandler extends dependencies['mail.model'] {
        ...
        async _handleNotificationChannelMessage(channelId, messageData) {
            ...
            // SwissCRM for users with notification "Handled by Email".
            ...
            // Chat from SwissCRMBot is considered disturbing and should only be
            // shown on the menu, but no notification and no thread open.
            const isChatWithSwissCRMBot = (
                channel.correspondent &&
                channel.correspondent === this.env.messaging.partnerRoot
            );
            if (!isChatWithSwissCRMBot) {
                const isSwissCRMFocused = this.env.services['bus_service'].isSwissCRMFocused();
                // Notify if out of focus
                if (!isSwissCRMFocused && channel.isChatChannel) {
