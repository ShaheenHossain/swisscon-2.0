...
from swiss.addons.crm.models import crm_stage
...
class CRMRevealRule(models.Model):
    ...
    industry_tag_ids = fields.Many2many('crm.iap.lead.industry', string='Industries', help='Leave empty to always match. SwissCRM will not create lead if no match')
    ...
    def _create_lead_from_response(self, result):
        ...
        template_values.update({
            'flavor_text': _("Opportunity created by SwissCRM Lead Generation"),
    ...
    # Methods responsible for format response data in to valid swiss lead data
