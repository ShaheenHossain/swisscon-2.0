...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/swiss" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/swiss/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/swiss/* /home/pi/swiss/
rm -r /home/pi/swiss/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service swiss restart) &
