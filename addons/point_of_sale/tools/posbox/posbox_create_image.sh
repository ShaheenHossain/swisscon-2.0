...
REPO=https://github.com/swiss/swiss.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/swiss"
...
swiss/
swiss-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/swiss
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/swiss/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
