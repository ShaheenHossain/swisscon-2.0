swiss.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousSwissCRM: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var swissSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var swissPromise = self._getSwissCRMSuggestions(value, isVAT).then(function (suggestions){
                swissSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with SwissCRM result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = swissSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            swissSuggestions.push(suggestion);
                        }
                    });
                }

                swissSuggestions = _.filter(swissSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(swissSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(swissSuggestions);
            };

            self._whenAll([swissPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use SwissCRM Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    _getSwissCRMSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousSwissCRM.add(def);
    },
    /**
