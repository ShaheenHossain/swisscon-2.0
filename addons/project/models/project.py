...
from swiss.exceptions import UserError, AccessError, ValidationError, RedirectWarning
from swiss.tools.misc import format_date, get_lang
from swiss.osv.expression import OR
...
class Task(models.Model):
    ...
    def _notify_email_header_dict(self):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-SwissCRM-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-SwissCRM-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-SwissCRM-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
