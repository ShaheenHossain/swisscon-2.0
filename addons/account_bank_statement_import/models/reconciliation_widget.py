...
from swiss.tools.misc import formatLang, format_date, parse_date
...
class AccountReconciliation(models.AbstractModel):
    ...
    def get_bank_statement_data(self, bank_statement_line_ids, srch_domain=[]):
        ...
        domain = [['id', 'in', bank_statement_line_ids], ('is_reconciled', '=', False)] + srch_domain
    ...
    def get_all_data_for_manual_reconciliation(self, partner_ids, account_ids):
        ...
        aml_ids = self._context.get('active_ids') and self._context.get(
            'active_model') == 'account.move.line' and tuple(self._context.get('active_ids'))
        if aml_ids:
    ...
    def get_data_for_manual_reconciliation(self, res_type, res_ids=None, account_type=None):
        ...
        for row in rows:
            ...
            rec_prop = aml_ids and self.env['account.move.line'].browse(
                aml_ids) or self._get_move_line_reconciliation_proposition(account.id, partner_id)
        ...
        # be reconciled.
        return [r for r in rows if r['reconciliation_proposition']] + [r for r in rows if
                                                                       not r['reconciliation_proposition']]
    ...
    @api.model
    def _domain_move_lines_for_reconciliation(self, st_line, aml_accounts, partner_id, excluded_ids=[],
