...
The manifest file serves to declare a python package as an SwissCRM module
...
``license`` (``str``, defaults: ``LGPL-3``)
    ...
    * `OEEL-1` (SwissCRM Enterprise Edition License v1.0)
    * `OPL-1` (SwissCRM Proprietary License v1.0)
    * `Other proprietary`

``category`` (``str``, default: ``Uncategorized``)
    classification category within SwissCRM, rough business domain for the module.
...
``depends`` (``list(str)``)
    SwissCRM modules which must be loaded before this one, either because this
...
.. _existing categories:
     https://github.com/swiss/swiss/blob/14.0/swiss/addons/base/data/ir_module_category_data.xml
