...
.. autoclass:: swiss.models.BaseModel()
...
.. autoclass:: swiss.models.AbstractModel()

Model
-----

.. autoclass:: swiss.models.Model()
...
.. autoclass:: swiss.models.TransientModel()
...
:class:`Dates <swiss.fields.Date>` and :class:`Datetimes <swiss.fields.Datetime>`
...
aims to provide SwissCRM developers with the knowledge required
...
* A string in the proper server format:
  ...
  * ``YYYY-MM-DD`` for :class:`~swiss.fields.Date` fields,
  * ``YYYY-MM-DD HH:MM:SS`` for :class:`~swiss.fields.Datetime` fields.
...
* :func:`~swiss.fields.Date.to_date` will convert to a :class:`datetime.date`
* :func:`~swiss.fields.Datetime.to_datetime` will convert to a :class:`datetime.datetime`.
...
:class:`~swiss.fields.Date` and :class:`~swiss.fields.Datetime`.
These helpers are also available by importing `swiss.tools.date_utils`.

.. note:: Timezones

    Datetime fields are stored as `timestamp without timezone` columns in the database and are stored
    in the UTC timezone. This is by design, as it makes the SwissCRM database independent from the timezone
...
.. warning::
    ...
    You cannot chain :class:`~swiss.fields.Many2many` or :class:`~swiss.fields.One2many` fields in ``related`` fields dependencies.

    ``related`` can be used to refer to a :class:`~swiss.fields.One2many` or
    :class:`~swiss.fields.Many2many` field on another model on the
...
.. attribute:: id
    ...
    Identifier :class:`field <swiss.fields.Field>`
...
:attr:`~swiss.models.BaseModel._log_access` is enabled. It can be
disabled to avoid creating or updating those fields on tables for which they are
not useful.

By default, :attr:`~swiss.models.BaseModel._log_access` is set to the same value
as :attr:`~swiss.models.BaseModel._auto`

.. attribute:: create_date

    Stores when the record was created, :class:`~swiss.fields.Datetime`

.. attribute:: create_uid

    Stores *who* created the record, :class:`~swiss.fields.Many2one` to a
    ``res.users``.

.. attribute:: write_date

    Stores when the record was last updated, :class:`~swiss.fields.Datetime`

.. attribute:: write_uid

    Stores who last updated the record, :class:`~swiss.fields.Many2one` to a
    ``res.users``.

.. warning:: :attr:`~swiss.models.BaseModel._log_access` *must* be enabled on
             :class:`~swiss.models.TransientModel`.
...
.. attribute:: name
  ...
  default value for :attr:`~swiss.models.BaseModel._rec_name`, used to
  display records in context where a representative "naming" is
  necessary.

  :class:`~swiss.fields.Char`
...
..   in list views.
..
..   :class:`~swiss.fields.Integer`

.. attribute:: state

  lifecycle stages of the object, used by the ``states`` attribute on
  :class:`fields <swiss.fields.Field>`.

  :class:`~swiss.fields.Selection`
...
.. attribute:: company_id
  ...
  Main field name used for SwissCRM multi-company behavior.

  Used by `:meth:~swiss.models._check_company` to check multi company consistency.
  Defines whether a record is shared between companies (no value) or only
  accessible by the users of a given company.

  :class:`~swiss.fields.Many2one`
  :type: :class:`~swiss.addons.base.models.res_company`
...
.. note::
    ...
    records, use :meth:`~swiss.models.BaseModel.mapped`::
...
Accessing a relational field (:class:`~swiss.fields.Many2one`,
:class:`~swiss.fields.One2many`, :class:`~swiss.fields.Many2many`)
*always* returns a recordset, empty if the field is not set.

Record cache and prefetching
----------------------------

SwissCRM maintains a cache for the fields of the records, so that not every field
...
To avoid reading one field on one record at a time, SwissCRM *prefetches* records
...
The :class:`~swiss.api.Environment` stores various contextual data used by
...
using :attr:`~swiss.models.Model.env` and gives access to:

* the current user (:attr:`~swiss.api.Environment.user`)
* the cursor (:attr:`~swiss.api.Environment.cr`)
* the superuser flag (:attr:`~swiss.api.Environment.su`)
* or the context (:attr:`~swiss.api.Environment.context`)
...
The :attr:`~swiss.api.Environment.cr` attribute on environments is the
...
Because models use the same cursor and the :class:`~swiss.api.Environment`
...
.. note::
    ...
    :meth:`~swiss.models.Model.invalidate_cache` method.

.. automethod:: Model.invalidate_cache

.. warning::
    Executing raw SQL bypasses the ORM, and by consequent, SwissCRM security rules.
...
* ``field_name`` (``str``)
    ...
    a :class:`~swiss.fields.Many2one` using dot-notation e.g. ``'street'``
...
* ``operator`` (``str``)
    ...
    ``child_of``
        ...
        :attr:`~swiss.models.Model._parent_name`).
    ``parent_of``
...
.. attribute:: env
    ...
    :type: :class:`~swiss.api.Environment`
...
SwissCRM provides three different mechanisms to extend models in a modular way:
...
When using the :attr:`~swiss.models.Model._inherit` and
:attr:`~swiss.models.Model._name` attributes together, SwissCRM creates a new
model using the existing one (provided via
:attr:`~swiss.models.Model._inherit`) as a base. The new model gets all the
fields, methods and meta-information (defaults & al) from its base.

.. literalinclude:: ../../swiss/addons/test_documentation_examples/inheritance.py
    :language: python
    :lines: 6-

and using them:

.. literalinclude:: ../../swiss/addons/test_documentation_examples/tests/test_inheritance.py
    :language: python
    :lines: 10,11,14,19
...
When using :attr:`~swiss.models.Model._inherit` but leaving out
:attr:`~swiss.models.Model._name`, the new model replaces the existing one,
...
.. literalinclude:: ../../swiss/addons/test_documentation_examples/extension.py
    :language: python
    :lines: 6-

.. literalinclude:: ../../swiss/addons/test_documentation_examples/tests/test_extension.py
    :language: python
    :lines: 10,15
...
at runtime) but less power: using the :attr:`~swiss.models.Model._inherits`
...
:class:`~swiss.fields.Reference` fields automatically set up on the parent
model.
...
.. literalinclude:: ../../swiss/addons/test_documentation_examples/delegation.py
    :language: python
    :lines: 5-

.. literalinclude:: ../../swiss/addons/test_documentation_examples/tests/test_delegation.py
    :language: python
    :lines: 11-14,23,28
