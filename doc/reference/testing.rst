...
There are many ways to test an application.  In SwissCRM, we have three kinds of
tests
...
SwissCRM provides support for testing modules using unittest.
...
`unittest documentation`_, but SwissCRM provides a number of utilities and helpers
related to testing SwissCRM content (modules, mainly):

.. autoclass:: swiss.tests.common.TransactionCase
    :members: browse_ref, ref

.. autoclass:: swiss.tests.common.SingleTransactionCase
    :members: browse_ref, ref

.. autoclass:: swiss.tests.common.SavepointCase

.. autoclass:: swiss.tests.common.HttpCase
    :members: browse_ref, ref, url_open, browser_js

.. autofunction:: swiss.tests.common.tagged
...
:class:`~swiss.tests.common.TransactionCase` and test a property of a model
in each method::
...
.. autoclass:: swiss.tests.common.Form
    :members:

.. autoclass:: swiss.tests.common.M2MProxy
    :members: add, remove, clear

.. autoclass:: swiss.tests.common.O2MProxy
...
:option:`--test-enable <swiss-bin --test-enable>` was enabled when starting the
SwissCRM server.
...
In SwissCRM, Python tests can be tagged to facilitate the test selection when
running tests.

Subclasses of :class:`swiss.tests.common.BaseCase` (usually through
:class:`~swiss.tests.common.TransactionCase`,
:class:`~swiss.tests.common.SavepointCase` or
:class:`~swiss.tests.common.HttpCase`) are automatically tagged with
``standard``, ``at_install`` and their source module's name by default.

Invocation
^^^^^^^^^^

:option:`--test-tags <swiss-bin --test-tags>` can be used to select/filter tests
to run on the command-line.

This option defaults to ``+standard`` meaning tests tagged ``standard``
(explicitly or implicitly) will be run by default when starting SwissCRM
with :option:`--test-enable <swiss-bin --test-enable>`.

When writing tests, the :func:`~swiss.tests.common.tagged` decorator can be
used on **test classes** to add or remove tags.

The decorator's arguments are tag names, as strings.

.. danger:: :func:`~swiss.tests.common.tagged` is a class decorator, it has no
...
.. code-block:: python
    ...
    from swiss.tests import TransactionCase, tagged
...
.. code-block:: console
    ...
    $ swiss-bin --test-enable --test-tags nice
...
:option:`--test-tags <swiss-bin --test-tags>`: on the command-line, values
are *additive* (you're selecting all tests with *any* of the specified tags)

.. code-block:: console

    $ swiss-bin --test-enable --test-tags nice,standard
...
.. code-block:: console
    ...
    $ swiss-bin --test-enable --test-tags 'standard,-slow'

When you write a test that does not inherit from the
:class:`~swiss.tests.common.BaseCase`, this test will not have the default tags,
...
- ``standard``: All SwissCRM tests that inherit from
  :class:`~swiss.tests.common.BaseCase` are implicitely tagged standard.
  :option:`--test-tags <swiss-bin --test-tags>` also defaults to ``standard``.
...
- *module_name*: SwissCRM tests classes extending
  :class:`~swiss.tests.common.BaseCase` are implicitely tagged with the
  ...
  .. code-block:: console
      ...
      $ swiss-bin --test-enable --test-tags stock_account
...
.. important::
    ...
    modules have to be selected with the :option:`-u <swiss-bin -u>` or
    :option:`-i <swiss-bin -i>` switches.  For simplicity, those switches are
...
.. code-block:: console
    ...
    $ swiss-bin --test-enable --test-tags sale

Run the tests from the sale module but not the ones tagged as slow:

.. code-block:: console

    $ swiss-bin --test-enable --test-tags 'sale,-slow'

Run only the tests from stock or tagged as slow:

.. code-block:: console

    $ swiss-bin --test-enable --test-tags '-standard, slow, stock'
...
guarantee that some basic functionality still works. Since SwissCRM has a non trivial
...
The SwissCRM framework uses the QUnit_ library testing framework as a test runner.
...
The main way to run the test suite is to have a running SwissCRM server, then
...
With the way SwissCRM is designed, any addon can modify the behaviour of other parts
...
1. create a new file *my_addon/static/tests/utils_tests.js*. This file contains the basic code to
   add a QUnit module *my_addon > utils*.
    ...
    .. code-block:: javascript
        ...
        swiss.define('my_addon.utils_tests', function (require) {
        "use strict";
...
Without help, it is quite difficult to test some parts of SwissCRM. In particular,
...
- Mock test functions: these functions help setting up a test environment. The
  most important use case is mocking the answers given by the SwissCRM server. These
...
- `qunit assertions`_: QUnit can be extended with specialized assertions. For
  SwissCRM, we frequently test some DOM properties. This is why we made some
...
.. code-block:: console
  ...
  '/tmp/swiss_tests/{db_name}/screenshots/'

Two new command line arguments were added since SwissCRM 13.0 to control this behavior:
:option:`--screenshots <swiss-bin --screenshots>` and :option:`--screencasts <swiss-bin --screencasts>`
...
you can use the :meth:`~swiss.tests.common.BaseCase.assertQueryCount` method, integrated in SwissCRM test classes.
...
SwissCRM CLI offers a :ref:`database population<reference/cmdline/populate>` feature.
...
.. note::
    ...
    You have to define at least :meth:`~swiss.models.Model._populate` or :meth:`~swiss.models.Model._populate_factories`
...
.. code-block:: python
    ...
    from swiss.tools import populate
...
.. automodule:: swiss.tools.populate
    :members: cartesian, compute, constant, iterate, randint, randomize

.. _qunit: https://qunitjs.com/
.. _qunit_config.js: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/qunit_config.js#L49
.. _web.tests_assets: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/views/webclient_templates.xml#L594
.. _web.qunit_suite: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/views/webclient_templates.xml#L660
.. _web.qunit_suite_tests: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/views/webclient_templates.xml#L680
.. _controller: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/controllers/main.py#L637
.. _test_js.py: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/tests/test_js.py#L13
.. _test_utils.js: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/test_utils.js
.. _mock server: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/mock_server.js
.. _qunit assertions: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/qunit_asserts.js
.. _createView: https://github.com/swiss/swiss/blob/51ee0c3cb59810449a60dae0b086b49b1ed6f946/addons/web/static/tests/helpers/test_utils_create.js#L267
