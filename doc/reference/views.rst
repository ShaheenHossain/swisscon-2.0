...
* ``groups_id`` :class:`~swiss.fields.Many2many` -> :class:`swiss.addons.base.models.res_users.Groups`
...
* ``banner_route``
  ...
  .. code-block:: python
      ...
      class MyController(swiss.http.Controller):
...
The two following :class:`~swiss.addons.base.ir_ui_view.View` fields are used to specify
inherited views.

* ``inherit_id`` :class:`~swiss.fields.Many2one`
...
* ``mode`` :class:`~swiss.fields.Selection`: `extension / primary`
...
``aggregate``
    ...
    - ``group_operator`` (optional)
        ...
        .. note:: The special aggregate function ``count_distinct`` (defined in swiss) can also be used here
...
Semantic components tie into and allow interaction with the SwissCRM
system. Available semantic components are:

``button``
  call into the SwissCRM system, similar to :ref:`list view buttons
  ...
  ``confirm``
    ...
    performing the button's SwissCRM call (also works in Kanban views).
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~swiss.fields.Char`,
    :class:`~swiss.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`swiss.api.onchange` on the model
  ...
  ``mode``
    for :class:`~swiss.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~swiss.fields.Char` field stores a password and
...
``examples``
  if set to a key in the `KanbanExamplesRegistry`_, examples on column setups will be available in the grouped kanban view. `Here <https://github.com/swiss/swiss/blob/99821fdcf89aa66ac9561a972c6823135ebf65c0/addons/project/static/src/js/project_task_kanban_examples.js#L27>`_ is an example of how to define those setups.
``group_create``
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~swiss.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/swiss/swiss/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``context``
  ...
  ``selection_mode``
    ...
    * buttons and links with a ``type`` attribute become perform SwissCRM-related
      operations rather than their standard HTML function. Possible types are:

      ``action``, ``object``
        standard behavior for :ref:`SwissCRM buttons
        <reference/views/list/button>`, most attributes relevant to standard
        SwissCRM buttons can be used.
      ``open``
...
``default_order``
    overrides the ordering of the view, replacing the model's order (:attr:`~swiss.models.BaseModel._order` model attribute).
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects SwissCRM:
    ...
    ``context``
        merged into the view's context when performing the button's SwissCRM call
...
``field``
    ...
    ``context``
        ...
        values e.g. ``[id_1, id_2]`` for a :class:`~swiss.fields.Many2one` field).
    ...
    ``domain``
        ...
        (e.g. :class:`~swiss.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain`` (optional)
        an SwissCRM :ref:`domain <reference/orm/domains>`, will be appended to the
    ...
    ``context``
        ...
        .. note::
           ...
           :class:`~swiss.fields.Field` attributes documentation.
...
.. _KanbanExamplesRegistry: https://github.com/swiss/swiss/blob/99821fdcf89aa66ac9561a972c6823135ebf65c0/addons/web/static/src/js/views/kanban/kanban_examples_registry.js
