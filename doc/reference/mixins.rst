...
SwissCRM implements some useful classes and mixins that make it easy for you to add
...
``_mail_post_access`` - :class:`~swiss.models.Model`  attribute
...
database without needing to connect to SwissCRM directly.
...
catchall domain will be sufficient since all routing will be done inside SwissCRM.
...
.. note:: Aliases can also be created manually and supported by a simple
    :class:`~swiss.fields.Many2one` field. This guide assumes you wish a
...
``alias_defaults`` - :class:`~swiss.fields.Text`
    Python dictionary that will be evaluated to provide
    default values when creating new records for this alias
``alias_force_thread_id`` - :class:`~swiss.fields.Integer`
    optional ID of a thread (record) to which all incoming messages will be
    attached, even if they did not reply to it; if set, this will disable the
    creation of new records completely
``alias_contact`` - :class:`~swiss.fields.Selection`
...
* ``campaign_id``: :class:`~swiss.fields.Many2one` field to a ``utm.campaign``
  object (i.e. Christmas_Special, Fall_Collection, etc.)
* ``source_id``: :class:`~swiss.fields.Many2one` field to a ``utm.source``
  object (i.e. Search Engine, mailing list, etc.)
* ``medium_id``: :class:`~swiss.fields.Many2one` field to a ``utm.medium``
    ...
    class MyModel(models.Models):
        ...
        def tracking_fields(self):
            ...
            # ("URL_PARAMETER", "FIELD_NAME_MIXIN", "NAME_IN_COOKIES")
                ('my_field', 'my_field', 'swiss_utm_my_field')
            ])
            return result

This will tell the system to create a cookie named *swiss_utm_my_field* with the
...
* ``helpdesk.ticket`` in the Helpdesk (*helpdesk* - SwissCRM Enterprise only) Application
...
* ``website_published``: :class:`~swiss.fields.Boolean` field which represents
  the status of the publication
* ``website_url``: :class:`~swiss.fields.Char` field which represents
...
* ``website_meta_title``: :class:`~swiss.fields.Char` field that allow you to set
  an additional title to your page
* ``website_meta_description``: :class:`~swiss.fields.Char` field that contains a
  short description of the page (sometimes used in search engines results)
* ``website_meta_keywords``: :class:`~swiss.fields.Char` field that contains some
