...
``views``
    ...
    :meth:`~swiss.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an SwissCRM action. Can be customized
via two fields:
...
will replace the current content section by the SwissCRM home page.
...
.. autoclass:: swiss.addons.base.models.ir_actions.IrActionsServer
...
``model_id``
    SwissCRM model linked to the action.
...
``fields_lines`` (create/write)
    ...
    :class:`~swiss.fields.One2many` with the fields:
...
``groups_id``
    :class:`~swiss.fields.Many2many` field to the groups allowed to view/use
    the current report
``multi``
    if set to ``True``, the action will not be displayed on a form view.
``paperformat_id``
    :class:`~swiss.fields.Many2one` field to the paper format you wish to
