...
.. note:: If you want to contribute to the translation of SwissCRM itself, please refer to the
  `SwissCRM Wiki page <https://github.com/swiss/swiss/wiki/Translations>`_.
...
automatically loaded by SwissCRM when the corresponding language is installed (via
...
SwissCRM automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~swiss.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~swiss.models.Model._constraints` and
  :attr:`~swiss.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, SwissCRM cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`swiss._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`swiss.web._t`:
...
The lazy version of `_` and `_t` is :func:`swiss._lt` in python and
:js:func:`swiss.web._lt` in javascript. The translation lookup is executed only
