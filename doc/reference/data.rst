...
SwissCRM is greatly data-driven, and a big part of modules definition is thus
...
The main way to define data in SwissCRM is via XML data files: The broad structure
of an XML data file is the following:

* Any number of operation elements within the root element ``swiss``
...
.. note::
    ...
    can specify the swiss flag ``noupdate`` set to 1.  If part of
    ...
    .. code-block:: xml
      ...
      <swiss>
          <data noupdate="1">
              <!-- Only loaded when installing the module (swiss-bin -i module) -->
              <operation/>
          </data>

          <!-- (Re)Loaded at install and update (swiss-bin -i/-u) -->
          <operation/>
      </swiss>
...
``search``
    ...
    the field is a :class:`~swiss.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~swiss.fields.Many2one` and
    :class:`~swiss.fields.Reference` fields
``type``
...
Because some important structural models of SwissCRM are complex and involved,
