...
===================================
Command-line interface: swiss-bin
===================================
...
.. option:: -s, --save
    ...
    (:file:`{$HOME}/.swissrc` by default, and can be overridden using
    :option:`-c`).
...
.. option:: --screenshots
    ...
    fails. It defaults to :file:`/tmp/swiss_tests/{db_name}/screenshots`
...
.. option:: --db-filter <filter>
    ...
    .. code-block:: bash
        ...
        $ swiss-bin --db-filter ^11.*$

    Restrict access to databases whose name starts with 11

    .. code-block:: bash

        $ swiss-bin --database 11firstdatabase,11seconddatabase

    Restrict access to only two databases, 11firstdatabase and 11seconddatabase

    .. code-block:: bash

        $ swiss-bin --database 11firstdatabase,11seconddatabase -u base
    ...
    .. code-block:: bash
        ...
        $ swiss-bin --db-filter ^11.*$ --database 11firstdatabase,11seconddatabase -u base
...
.. option:: --db_sslmode
    ...
    Control the SSL security of the connection between SwissCRM and PostgreSQL.
...
.. option:: --email-from <address>
    ...
    Email address used as <FROM> when SwissCRM needs to send mails
...
.. option:: --smtp-ssl
    ...
    If set, swiss should use SSL/STARTSSL SMTP connections
...
Use these options to translate SwissCRM to another language. See i18n section of
...
By default, SwissCRM displays all logging of level_ ``info`` except for workflow
...
.. option:: --log-handler <handler-spec>
    ...
    e.g. ``swiss.models:DEBUG`` will enable all logging messages at or above
    ...
    * If the level is omitted, the logger is set to ``INFO``
    ...
    .. code-block:: console
        ...
        $ swiss-bin --log-handler :DEBUG --log-handler werkzeug:CRITICAL --log-handler swiss.fields:WARNING

.. option:: --log-request

    enable DEBUG logging for RPC requests, equivalent to
    ``--log-handler=swiss.http.rpc.request:DEBUG``

.. option:: --log-response

    enable DEBUG logging for RPC responses, equivalent to
    ``--log-handler=swiss.http.rpc.response:DEBUG``

.. option:: --log-web

    enables DEBUG logging of HTTP requests and responses, equivalent to
    ``--log-handler=swiss.http:DEBUG``

.. option:: --log-sql

    enables DEBUG logging of SQL querying, equivalent to
    ``--log-handler=swiss.sql_db:DEBUG``
...
.. option:: --log-level <level>
    ...
    ``swiss`` and ``werkzeug`` loggers (except for ``debug`` which is only
    set on ``swiss``).

    SwissCRM also provides debugging pseudo-levels which apply to different sets
    of loggers:
    ...
    ``debug_rpc``
        sets the ``swiss`` and HTTP request loggers to ``debug``

        equivalent to ``--log-level debug --log-request``
    ``debug_rpc_answer``
        sets the ``swiss`` and HTTP request and response loggers to
        ``debug``
...
The default configuration file is :file:`{$HOME}/.swissrc` which
can be overridden using :option:`--config <swiss-bin -c>`. Specifying
:option:`--save <swiss-bin -s>` will save the current configuration state back
...
SwissCRM command-line also allows to launch swiss as a python console environment.
...
.. program:: swiss-bin scaffold

Scaffolding is the automated creation of a skeleton structure to simplify
bootstrapping (of new modules, in the case of SwissCRM). While not necessary it
avoids the tedium of setting up basic structures and looking up what all
starting requirements are.

Scaffolding is available via the :command:`swiss-bin scaffold` subcommand.

.. code-block:: console

    $ swiss_bin scaffold my_module /addons/
...
.. program:: swiss-bin populate

SwissCRM CLI supports database population features. If the feature is
...
.. option:: --size (small|medium|large)
    ...
    The generated records content is specified by the :meth:`~swiss.models._populate_factories` method
...
.. program:: swiss-bin cloc

SwissCRM Cloc is a tool to count the number of relevant lines written in
...
.. code-block:: console
   ...
   $ swiss-bin cloc --addons-path=addons -d my_database
...
.. code-block:: console
   ...
   $ swiss-bin cloc -p addons/account
...
.. code-block:: console
   ...
   $ swiss-bin cloc -p addons/account -p addons/sale
...
.. code-block:: console
    ...
    $ swiss-bin cloc -c config.conf -d my_database
...
SwissCRM Cloc counts the lines in each file of extra installed modules in a
...
For special cases, a list of files that should be ignored by SwissCRM Cloc can be defined
...
To distinguish between standard and extra modules, SwissCRM Cloc uses the following heuristic:
...
Some file cannot be counted by SwissCRM Cloc.
Those file are reported at the end of the output.

Max file size exceeded
''''''''''''''''''''''

SwissCRM Cloc rejects any file larger than 25MB. Usually, source files are smaller
...
SwissCRM Cloc cannot count the lines of code of a Python file with a syntax problem.
