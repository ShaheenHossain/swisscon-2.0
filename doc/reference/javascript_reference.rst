...
This document presents the SwissCRM Javascript framework. This
...
- the *website*: this is the public part of SwissCRM.  It allows an unidentified
...
It means that while a user is working on SwissCRM, the web client class (and the
...
Managing assets in SwissCRM is not as straightforward as it is in some other apps.
...
a collection of files (javascript, css, scss). In SwissCRM, the most important
...
Main bundles
------------
When the SwissCRM server is started, it checks the timestamp of each file in a bundle,
...
- *web.assets_common*: this bundle contains most assets which are common to the
  ...
  lower level building blocks for the swiss framework.  Note that it contains the
  *boot.js* file, which defines the swiss module system.
...
.. note ::
    ...
    swiss web client.  This means that the files are transferred through the network
...
sure they are loaded in the correct order.  In order to do that, SwissCRM has defined
a small module system (located in the file *addons/web/static/src/js/boot.js*,
which needs to be loaded first).

The SwissCRM module system, inspired by AMD, works by defining the function *define*
on the global swiss object. We then define each javascript module by calling that
function.  In the SwissCRM framework, a module is a piece of code that will be executed
...
.. code-block:: javascript
    ...
    // in file a.js
    swiss.define('module.A', function (require) {
        "use strict";
    ...
    // in file b.js
    swiss.define('module.B', function (require) {
        "use strict";
...
.. code-block:: javascript
    ...
    swiss.define('module.Something', ['module.A', 'module.B'], function (require) {
        "use strict";
...
The *swiss.define* method is given three arguments:

- *moduleName*: the name of the javascript module.  It should be a unique string.
  The convention is to have the name of the swiss addon followed by a specific
...
.. code-block:: javascript
      ...
      swiss.define('module.Something', ['web.ajax'], function (require) {
        "use strict";
...
.. code-block:: javascript
    ...
    swiss.define('module.Something', function (require) {
        "use strict";
...
SwissCRM was developed before ECMAScript 6 classes were available.  In Ecmascript 5,
...
For these reasons, SwissCRM decided to use its own class system, inspired by John
...
The swiss Class system does not support multiple inheritance, but for those cases
...
the way SwissCRM is structured, it is sometimes necessary in one addon to modify
...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in SwissCRM is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  SwissCRM Web's
...
There are currently two event systems supported by SwissCRM: a simple system which
...
- *trigger_up*: this is the method that will create a small *SwissCRMEvent* and
...
The SwissCRMEvent class is very simple.  It has three public attributes: *target*
...
A common need in the SwissCRM ecosystem is to extend/change the behaviour of the
...
There are typically two usecases when working on SwissCRM: one may need to call a
...
The SwissCRM framework has a standard way to communicate various information to the
...
The notification system in SwissCRM is designed with the following components:
...
In 2017, SwissCRM replaced the previous view code with a new architecture.  The
...
handled by swiss and correspond to italic and bold, respectively).  Note that the
...
For more information, look into the `control_panel_renderer.js <https://github.com/swiss/swiss/blob/13.0/addons/web/static/src/js/views/control_panel/control_panel_renderer.js#L130>`_ file.
