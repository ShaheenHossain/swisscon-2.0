...
IoT Drivers allow any SwissCRM module to communicate in real-time with any device
connected to the IoT Box. Communication with the IoT Box goes both ways, so the
SwissCRM client can send commands to and receive information from any of the
supported devices.
...
be located on the connected SwissCRM instance. Each module can contain an
...
through its IP address and in the IoT module of the connected SwissCRM instance.
...
.. code-block:: python
    ...
    from swiss.addons.hw_drivers.interface import Interface
...
from an SwissCRM module. It takes the data given during the call as argument.
...
When any module in SwissCRM wants to read data from a specific device, it creates a
