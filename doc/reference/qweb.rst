...
QWeb is the primary templating_ engine used by SwissCRM\ [#othertemplates]_. It
...
:meth:`swiss.http.HttpRequest.render`:
...
This automatically creates a :class:`~swiss.http.Response` object which can
...
.. py:method:: render(cr, uid, id[, values][, engine='ir.qweb][, context])
    ...
    ``request``
        the current :class:`~swiss.http.WebRequest` object, if any
    ``debug``
    ...
    :param str engine: name of the SwissCRM model to use for rendering, can be
...
.. .. automodule:: swiss.addons.base.ir.ir_qweb
...
.. js:class:: QWeb2.Engine
    ...
    SwissCRM Web instantiates one for the user in the core module, and
    ...
    SwissCRM Web, Kanban views get their own :js:class:`QWeb2.Engine`
    ...
    .. js:attribute:: QWeb2.Engine.preprocess_node
        ...
        node to template code. In SwissCRM Web, this is used to
...
.. [#othertemplates] although it uses a few others, either for historical
                     ...
                     use case. SwissCRM 9.0 still depends on Jinja_ and Mako_.
