...
There are many ways to solve a problem in JavaScript, and in SwissCRM.  However, the
SwissCRM framework was designed to be extensible (this is a pretty big constraint),
and some common problems have a nice standard solution.  The standard solution
has probably the advantage of being easy to understand for an swiss developers,
and will probably keep working when SwissCRM is modified.
...
First of all, remember that the first rule of customizing swiss with JS is:
...
example, the voip addon in swiss need to modify the FieldPhone widget to add the
...
- in SwissCRM specifically
        In SwissCRM, it happens that we use promise rejection for control flow, like in mutexes and other concurrency primitives defined in module `web.concurrency`
