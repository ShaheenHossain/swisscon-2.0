        ...
        for title, ref in ((e[0], e[1]) for e in node['entries']):
            ...
            for subtitle, subref in entries:
                ...
                if subref in env.metadata:
                    cover = env.metadata[subref].get('banner', conf.swiss_cover_default)
                elif subref in conf.swiss_cover_external:
                    cover = conf.swiss_cover_external[subref]
                else:
                    cover = conf.swiss_cover_default_external
