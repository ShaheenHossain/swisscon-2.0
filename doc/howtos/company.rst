...
.. warning::
    ...
    This tutorial requires good knowledge of SwissCRM.
...
To correctly manage multi-company behaviors, SwissCRM's ORM provides multiple features:
...
.. note:: The `_compute_display_info` method is decorated with `depends_context('company')`
          (see :attr:`~swiss.api.depends_context`) to ensure that the computed field is recomputed
...
to ensure the company we are using is the correct one.  This can be done with :meth:`~swiss.models.Model.with_company`,
...
On each :meth:`~swiss.models.Model.create` and :meth:`~swiss.models.Model.write`, automatic checks
