...
This guide is about creating modules for SwissCRM's web client.

To create websites with SwissCRM, see :doc:`website`; to add business capabilities
or extend existing business systems of SwissCRM, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed SwissCRM <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple SwissCRM module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/swiss/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to SwissCRM's
:option:`addons path <swiss-bin --addons-path>`, create a new database and
...
Files used in the "web" side of an SwissCRM module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        swiss.oepetstore = function(instance, local) {
...
.. warning::
    ...
    "developer mode": log into your SwissCRM instance (user *admin* password
    *admin* by default) open the setting menu and select :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_swiss.png
        :align: center
...
The SwissCRM framework uses one such pattern to define modules within web addons,
...
In SwissCRM web, modules are declared as functions set on the global ``swiss``
...
* the first parameter is the current instance of the SwissCRM web client, it gives
  access to various capabilities defined by the SwissCRM (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the SwissCRM web client needs to call them
...
For simplicity and developer-friendliness SwissCRM web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~swiss.web.Class.extend`
method of :class:`swiss.web.Class`::
...
The :func:`~swiss.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~swiss.web.Class.extend` on the parent class, as is done
to subclass :class:`~swiss.web.Class`::
...
The SwissCRM web client bundles jQuery_ for easy DOM manipulation. It is useful
...
SwissCRM Web makes specific components responsible for sections of a page. In
SwissCRM web, the base for such components is the :class:`~swiss.Widget`
...
It extends :class:`~swiss.Widget` and overrides the standard method
:func:`~swiss.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~swiss.Widget.start`
...
All widgets have a :attr:`~swiss.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~swiss.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear SwissCRM user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in SwissCRM Web, you will need to reload
    the page. There is no need to restart the SwissCRM server.

The ``HomePage`` widget is used by SwissCRM Web and managed automatically.
...
``GreetingsWidget``'s :func:`~swiss.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~swiss.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~swiss.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~swiss.Widget.start` method will be called during the call
to :func:`~swiss.Widget.appendTo`.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear SwissCRM user!</div>
...
:class:`~swiss.Widget`, because we added some classes on them.
...
When overriding the :func:`~swiss.Widget.init` method of a widget it is
...
it. This is done via the :func:`~swiss.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~swiss.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~swiss.Widget.init` or :func:`~swiss.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~swiss.Widget.destroy`.

.. danger::

    when overriding :func:`~swiss.Widget.destroy`, ``_super()``
...
As many other environments, SwissCRM's solution is to use a `template engine`_.
SwissCRM's template engine is called :ref:`reference/qweb`.
...
* It has special support in SwissCRM Web's :class:`~swiss.Widget`, though it
  can be used outside of SwissCRM's web client (and it's possible to use
  :class:`~swiss.Widget` without relying on QWeb)
...
.. note::
    ...
    like SwissCRM :ref:`views <reference/views>`.
    ...
    characteristic of SwissCRM, and losing it was considered unacceptable.
...
However, because :class:`~swiss.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~swiss.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~swiss.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~swiss.Widget.template`. If there are
...
When using :class:`~swiss.Widget`'s integration it is not possible to
...
before :func:`~swiss.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~swiss.Widget.init`):
...
But because it's a common operation, :class:`~swiss.Widget` provides an
equivalent shortcut through the :func:`~swiss.Widget.$` method::
...
:attr:`~swiss.Widget.events` is an object (mapping) of an event to the
...
:func:`~swiss.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~swiss.Widget.on`::
...
:func:`~swiss.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~swiss.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``swiss.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the SwissCRM web application.
...
* :func:`~swiss.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~swiss.Widget.set`) and ``change``
* :func:`~swiss.Widget.get` retrieves the value of a property.
...
The class system of the SwissCRM web framework allows direct modification of
existing classes using the :func:`~swiss.web.Class.include` method::
...
defined in the call to :func:`~swiss.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~swiss.Widget.include`.
...
In SwissCRM, translations files are automatically generated by scanning the source
...
:func:`~swiss.web._t` (and also :func:`~swiss.web._lt`).
...
:func:`~swiss.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the SwissCRM Server
=====================================

Contacting Models
-----------------

Most operations with SwissCRM involve communicating with *models* implementing
...
communicating with SwissCRM requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, SwissCRM web provides
...
The class used to call SwissCRM models is :class:`swiss.Model`. It is
instantiated with the SwissCRM model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~swiss.web.Model.call` can be used to call any (public) method of an
SwissCRM model. It takes the following positional arguments:
...
:func:`~swiss.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if SwissCRM
...
solution to achieve that is to use :class:`swiss.web.CompoundContext`.

:class:`~swiss.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~swiss.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in SwissCRM plus the
``new_key`` key that was added when instantiating
:class:`~swiss.web.CompoundContext`.

Queries
-------

While :func:`~swiss.Model.call` is sufficient for any interaction with SwissCRM
models, SwissCRM Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~swiss.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~swiss.models.Model.search` and
::py:meth:`~swiss.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~swiss.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`swiss.web.Query` which can be further customized before
  being executed
* :class:`~swiss.web.Query` represents the query being built. It is
  ...
  :class:`~swiss.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~swiss.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~swiss.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In SwissCRM, images are generally stored in regular fields encoded as
...
In SwissCRM, many operations start from an :ref:`action <reference/actions>`:
...
In SwissCRM Web, the component responsible for handling and reacting to these
...
:func:`~swiss.Widget.do_action` is a shortcut of :class:`~swiss.Widget`
...
the SwissCRM web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for SwissCRM web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~swiss.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~swiss.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the SwissCRM web client root.
...
Much of SwissCRM web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`SwissCRM views <reference/views>` are implemented through a subclass
of :class:`swiss.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main SwissCRM framework, but
...
:py:class:`~swiss.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~swiss.web.View.view_id` attribute.

Views are also provided with a :class:`~swiss.web.DataSet` instance which
...
:func:`~swiss.web.View.do_search`, and updating their
:class:`~swiss.web.DataSet` as necessary.
...
:py:class:`~swiss.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of SwissCRM. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in SwissCRM have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the SwissCRM server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in SwissCRM. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the SwissCRM web
client.
...
user to modify it can be useful, but most fields in SwissCRM also allow editing.
...
their :attr:`~swiss.web.form.FormWidget.field_manager` attribute::
...
:attr:`~swiss.web.form.FormWidget` is generally the
:class:`~swiss.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~swiss.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~swiss.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~swiss.web.form.FieldManagerMixin.set_values(values)` sets multiple
