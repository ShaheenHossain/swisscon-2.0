...
.. warning::
    ...
    * This guide assumes :ref:`an installed SwissCRM <setup/install>`

Creating a basic module
=======================

In SwissCRM, tasks are performed by creating modules.

Modules customize the behavior of an SwissCRM installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`SwissCRM's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./swiss-bin scaffold Academy my-modules
...
* start the SwissCRM server

  .. code-block:: console

      $ ./swiss-bin --addons-path addons,my-modules
...
display logic. SwissCRM allows any Python templating system, but provides its
...
Finally restart SwissCRM and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, SwissCRM can be restarted :option:`and update modules at
    the same time<swiss-bin -u>`:

    .. code-block:: console

        $ swiss-bin --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:8069/academy/academy/ should now result in:

.. image:: website/swiss_basic-list.png

Storing data in SwissCRM
=======================

:ref:`SwissCRM models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~swiss.models.Model.search` returns a set of records
...
SwissCRM bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but SwissCRM 8 added deeper
...
.. image:: website/swiss_layout.png
...
You're now in SwissCRM "proper": the administrative interface. For now click on
...
:func:`~swiss.http.route` decorator which takes a routing string and a
...
restart SwissCRM, access http://localhost:8069/academy/Alice/ and
...
Restart SwissCRM, access http://localhost:8069/academy/2, note how the old value
...
SwissCRM provides an additional converter called ``model`` which provides records
...
Restart SwissCRM and upgrade the module, then you can visit each teacher's page.
...
Restart SwissCRM and update the views, reload the teacher's page and… the field
...
Restart SwissCRM and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the SwissCRM administration
-----------------------------------------------------------------

The SwissCRM administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the SwissCRM backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which SwissCRM should
   execute or data display. Data display actions are called *window actions*,
   and tell SwissCRM to display a given *model* according to a set of views…
...
Editing in the SwissCRM administration
-------------------------------------

By default, an SwissCRM model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) SwissCRM will automatically create a basic one
...
:class:`the inverse relationship <swiss.fields.One2many>` to the *teachers*
model:
...
SwissCRM provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of SwissCRM's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~swiss.models.Model._inherit`
...
SwissCRM also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the SwissCRM system. We can easily make
...
restart SwissCRM, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <swiss.models.Model._inherit>` without giving it a new
      :attr:`~swiss.models.Model._name`
