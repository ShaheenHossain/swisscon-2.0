...
Start/Stop the SwissCRM server
=============================

SwissCRM uses a client/server architecture in which clients are web browsers
accessing the SwissCRM server via RPC.
...
In order to start the server, simply invoke the command :ref:`swiss-bin
...
Build an SwissCRM module
=======================

Both server and client extensions are packaged as *modules* which are
optionally loaded in a *database*.

SwissCRM modules can either add brand new business logic to an SwissCRM system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to SwissCRM's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in SwissCRM thus starts and ends with modules.

Composition of a module
-----------------------

An SwissCRM module can contain a number of elements:

Business objects
    Declared as Python classes, these resources are automatically persisted
    by SwissCRM based on their configuration
...
are specified by using the :option:`--addons-path <swiss-bin --addons-path>`
option.
...
An SwissCRM module is declared by its :ref:`manifest <reference/module/manifest>`.
...
SwissCRM provides a mechanism to help set up a new module, :ref:`swiss-bin
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ swiss-bin scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    install it in SwissCRM.

    .. only:: solutions

        #. Invoke the command ``swiss-bin scaffold openacademy addons``.
...
A key component of SwissCRM is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~swiss.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~swiss.models.Model._name` which is required and defines the name for
the model in the SwissCRM system. Here is a minimally complete definition of a
model::
...
:attr:`~swiss.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~swiss.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~swiss.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~swiss.fields.Field.index` (``bool``, default: ``False``)
    Requests that SwissCRM create a `database index`_ on the column.
...
Example of simple fields are :class:`~swiss.fields.Boolean`,
:class:`~swiss.fields.Date`, :class:`~swiss.fields.Char`.

Reserved fields
###############

SwissCRM creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~swiss.fields.Model.id` (:class:`~swiss.fields.Id`)
    The unique identifier for a record in its model.
:attr:`~swiss.fields.Model.create_date` (:class:`~swiss.fields.Datetime`)
    Creation date of the record.
:attr:`~swiss.fields.Model.create_uid` (:class:`~swiss.fields.Many2one`)
    User who created the record.
:attr:`~swiss.fields.Model.write_date` (:class:`~swiss.fields.Datetime`)
    Last modification date of the record.
:attr:`~swiss.fields.Model.write_uid` (:class:`~swiss.fields.Many2one`)
    user who last modified the record.

Special fields
##############

By default, SwissCRM also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~swiss.models.Model._rec_name`.
...
SwissCRM is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into SwissCRM
    :class: aphorism
...
* ``model`` is the name of the SwissCRM model for the record.
...
.. tip:: The content of the data files is only loaded when a module is
    ...
    :ref:`swiss-bin -u openacademy <reference/cmdline>` to save the changes
    to your database.
...
If no search view exists for the model, SwissCRM generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <swiss.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <swiss.fields.One2many>`
    A virtual relationship, inverse of a :class:`~swiss.fields.Many2one`.
    A :class:`~swiss.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~swiss.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~swiss.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <swiss.fields.Many2many>`
...
SwissCRM provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~swiss.models.Model._inherit`
    * :attr:`~swiss.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), SwissCRM
...
In SwissCRM, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~swiss.fields.Field.compute` to the name of a method. The computation
...
on the compute method with the decorator :func:`~swiss.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            SwissCRM has built-in rules making records with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~swiss.api.onchange`
...
SwissCRM provides two ways to set up automatically verified invariants:
:func:`Python constraints <swiss.api.constrains>` and
:attr:`SQL constraints <swiss.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~swiss.api.constrains`, and invoked on a recordset. The decorator
...
:attr:`~swiss.models.Model._sql_constraints`. The latter is assigned to a list
...
.. exercise:: Add access control through the SwissCRM interface
...
the same convention as the method :meth:`~swiss.models.Model.write` of the ORM.
...
:class:`~swiss.models.TransientModel` instead of
:class:`~swiss.models.Model`. The class
:class:`~swiss.models.TransientModel` extends :class:`~swiss.models.Model`
...
pt_BR.po). Translations will be loaded automatically by SwissCRM for all
enabled languages. Developers always use English when creating a module, then
export the module terms using SwissCRM's gettext POT export feature
...
.. tip:: The Portable Object files generated by SwissCRM are published on
         `Transifex <https://www.transifex.com/swiss/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from SwissCRM)
...
.. tip::
   ...
   By default SwissCRM's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`swiss._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your SwissCRM installation. Translate your
   module using the facilities provided by SwissCRM.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``swiss._`` and mark missing strings as translatable
...
SwissCRM uses a report engine based on :ref:`reference/qweb`,
...
SwissCRM is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python 3 program that interacts with an SwissCRM
...
.. exercise:: Add a new service to the client
   ...
   SwissCRM (yours, or your instructor's). This program should display all
...
The following example is a Python 3 program that interacts with an SwissCRM server
...
.. note::
    ...
    There are a number of high-level APIs in various languages to access SwissCRM
    systems without *explicitly* going through XML-RPC or JSON-RPC, such as:

    * https://github.com/akretion/ooor
    * https://github.com/OCA/swissrpc
    * https://github.com/nicolas-van/openerp-client-lib
    * http://pythonhosted.org/SwissCRMRPC
...
.. [#rawsql] writing raw SQL queries is possible, but requires care as it
             bypasses all SwissCRM authentication and security mechanisms.
