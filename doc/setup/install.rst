...
There are multiple ways to install SwissCRM, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use SwissCRM in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing SwissCRM, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running SwissCRM versions on
...
To simply get a quick idea of SwissCRM, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by SwissCRM S.A., SwissCRM's SaaS_
provides private instances and starts out free. It can be used to discover and
test SwissCRM and do non-code customizations (i.e. incompatible with custom modules
or the SwissCRM Apps Store) without having to install it locally.

Can be used for both testing SwissCRM and long-term production use.
...
SwissCRM provides packaged installers for Windows, deb-based distributions
...
SwissCRM will automatically be started at the end of the installation.
...
SwissCRM 13.0 'deb' package currently supports `Debian Buster`_, `Ubuntu 18.04`_ or above.

Prepare
^^^^^^^

SwissCRM needs a `PostgreSQL`_ server to run properly. The default configuration for
the SwissCRM 'deb' package is to use the PostgreSQL server on the same host as your
SwissCRM instance. Execute the following command in order to install the PostgreSQL server:
...
.. warning:: `wkhtmltopdf` is not installed through **pip** and must be installed manually in
             ...
             footers. See our `wiki <https://github.com/swiss/swiss/wiki/Wkhtmltopdf>`_ for more
             details on the various versions.

Repository
^^^^^^^^^^

SwissCRM S.A. provides a repository that can be used with  Debian and Ubuntu distributions. It can be
used to install *SwissCRM Community Edition* by executing the following commands **as root**:
...
This will install SwissCRM as a service, create the necessary PostgreSQL_ user
...
.. warning:: The `num2words` python package does not exists in Debian Buster nor Ubuntu 18.04.
             Textual amounts will not be rendered by SwissCRM and this could cause problems with the
             `l10n_mx_edi` module.
...
SwissCRM 13.0 'rpm' package supports Fedora 30.

Prepare
^^^^^^^
SwissCRM needs a `PostgreSQL`_ server to run properly. Make sure that the `sudo` command is available
...
SwissCRM S.A. provides a repository that can be used with the Fedora distributions.
It can be used to install *SwissCRM Community Edition* by executing the following
commands:
...
.. code-block:: console
    ...
    $ sudo dnf localinstall swiss_13.0.latest.noarch.rpm
    $ sudo systemctl enable swiss
    $ sudo systemctl start swiss
...
The source "installation" is really about not installing SwissCRM, and running it directly from source
instead.

This can be more convenient for module developers as the SwissCRM source is more easily accessible
than using packaged installation (for information or to build this documentation and have it
available offline).

It also makes starting and stopping SwissCRM more flexible and explicit than the services set up by the
...
(and run) multiple versions of SwissCRM side-by-side.
...
There are two ways to obtain the source code of SwissCRM: as a zip **archive** or through **git**.
...
.. code-block:: doscon
    ...
    C:\> git clone https://github.com/swiss/swiss.git
...
.. code-block:: doscon
  ...
  C:\> git clone https://github.com/swiss/enterprise.git

.. note:: **The Enterprise git repository does not contain the full SwissCRM source code**. It is only
          ...
          both the Community and Enterprise repository to have a working SwissCRM Enterprise
          installation.
...
SwissCRM requires Python 3.6 or later to run. Visit `Python's download page <https://www.python.org/downloads/windows/>`_
...
.. note:: If Python 3 is already installed, make sure that the version is 3.6 or above, as previous
          versions are not compatible with SwissCRM.
...
SwissCRM uses PostgreSQL as database management system. `Download and install PostgreSQL <https://www.postgresql.org/download/windows/>`_
(supported version: 10.0 and later).

By default, the only user is `postgres` but SwissCRM forbids connecting as `postgres`, so you need to
...
#. Create a postgres user with a password using the pg admin gui:
   ...
   4. Enter the username in the **Role Name** field (e.g. `swiss`).
   5. Open the **Definition** tab and enter the password (e.g. ``swiss``), then click **Save**.
...
SwissCRM dependencies are listed in the `requirements.txt` file located at the root of the SwissCRM
community directory.

.. tip:: It can be preferable to not mix python modules packages between different instances of SwissCRM
         or with your system. You can use virtualenv_ to create isolated Python environments.

Navigate to the path of your SwissCRM Community installation (`CommunityPath`) and run **pip**
...
Once all dependencies are set up, SwissCRM can be launched by running `swiss-bin`, the
command-line interface of the server. It is located at the root of the SwissCRM Community directory.
...
.. code-block:: doscon
    ...
    C:\> cd CommunityPath/
    C:\> python swiss-bin -r dbuser -w dbpassword --addons-path=addons -d mydb

Where `CommunityPath` is the path of the SwissCRM Community installation, `dbuser` is the
...
.. code-block:: console
    ...
    $ git clone https://github.com/swiss/swiss.git
...
.. code-block:: console
  ...
  $ git clone https://github.com/swiss/enterprise.git
...
SwissCRM requires Python 3.6 or later to run. Use your package manager to download and install Python 3
...
SwissCRM uses PostgreSQL as database management system. Use your package manager to download and install
...
dependencies before the Python dependencies of SwissCRM. They are available in `-dev` or `-devel`
...
* PostgreSQL user and password. SwissCRM has no defaults beyond
...
.. code-block:: console
    ...
    $ cd /CommunityPath
    $ python3 swiss-bin --addons-path=addons -d mydb

Where `CommunityPath` is the path of the SwissCRM Community installation
and `mydb` is the default database to serve on `localhost:8069`. You can add other
directory paths separated by a comma to ``addons`` at the end of the addons-path option.
...
SwissCRM requires Python 3.6 or later to run. Use your preferred package manager (homebrew_, macports_)
...
SwissCRM uses PostgreSQL as database management system. Use `postgres.app <https://postgresapp.com>`_
...
The full documentation on how to use SwissCRM with Docker can be found on the
