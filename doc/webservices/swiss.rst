...
SwissCRM is usually extended internally via modules, but many of its features and
all of its data are also available from the outside for external analysis or
integration with various tools. Part of the :ref:`reference/orm/model` API is
easily available over XML-RPC_ and accessible from a variety of languages.

.. SwissCRM XML-RPC idiosyncracies:
   * uses multiple endpoint and a nested call syntax instead of a
     "hierarchical" server structure (e.g. ``swiss.res.partner.read()``)
...
If you already have an SwissCRM server installed, you can just use its
parameters
...
SwissCRM has support for **api keys** and (depending on modules or settings) may
...
To make exploration simpler, you can also ask https://demo.f2.swisshq.org for a test
database:
...
.. switcher::
    ...
    .. code-block:: python3
        ...
        import xmlrpc.client
        info = xmlrpc.client.ServerProxy('https://demo.f2.swisshq.org/start').start()
        url, db, username, password = \
            info['host'], info['database'], info['user'], info['password']
    ...
    .. code-block:: ruby
        ...
        require "xmlrpc/client"
        info = XMLRPC::Client.new2('https://demo.f2.swisshq.org/start').call('start')
        url, db, username, password = \
            info['host'], info['database'], info['user'], info['password']
    ...
    .. case:: PHP
        ...
        .. code-block:: php
            ...
            require_once('ripcord.php');
            $info = ripcord::client('https://demo.f2.swisshq.org/start')->start();
            list($url, $db, $username, $password) =
              array($info['host'], $info['database'], $info['user'], $info['password']);
    ...
    .. case:: Java
        ...
        .. code-block:: java
            ...
            final XmlRpcClientConfigImpl start_config = new XmlRpcClientConfigImpl();
            start_config.setServerURL(new URL("https://demo.f2.swisshq.org/start"));
            final Map<String, String> info = (Map<String, String>)client.execute(
                start_config, "start", emptyList());
...
SwissCRM requires users of the API to be authenticated before they can query most
data.
...
.. _webservices/swiss/calling_methods:

Calling methods
===============

The second endpoint is ``xmlrpc/2/object``, is used to call methods of swiss
...
Records can be listed and filtered via :meth:`~swiss.models.Model.search`.

:meth:`~swiss.models.Model.search` takes a mandatory
...
:meth:`~swiss.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~swiss.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~swiss.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~swiss.models.Model.search`) and optionally a list of fields to
...
:meth:`~swiss.models.Model.fields_get` can be used to inspect
...
Because it is a very common task, SwissCRM provides a
:meth:`~swiss.models.Model.search_read` shortcut which as its name suggests is
equivalent to a :meth:`~swiss.models.Model.search` followed by a
:meth:`~swiss.models.Model.read`, but avoids having to perform two requests
and keep ids around.

Its arguments are similar to :meth:`~swiss.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~swiss.models.Model.read`,
...
Records of a model are created using :meth:`~swiss.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~swiss.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~swiss.fields.Integer`, string for :class:`~swiss.fields.Char`
    or :class:`~swiss.fields.Text`),

    * :class:`~swiss.fields.Date`, :class:`~swiss.fields.Datetime` and
      :class:`~swiss.fields.Binary` fields use string values
    * :class:`~swiss.fields.One2many` and :class:`~swiss.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <swiss.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~swiss.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~swiss.models.Model.create`.
...
:meth:`~swiss.models.Model.unlink`.
...
While we previously used :meth:`~swiss.models.Model.fields_get` to query a
model and have been using an arbitrary model from the start, SwissCRM stores
...
Provides information about SwissCRM models via its various fields
...
``field_id``
    list of the model's fields through a :class:`~swiss.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~swiss.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~swiss.fields.One2many` relation to the
...
Provides information about the fields of SwissCRM models and allows adding
custom fields without using Python code

``model_id``
    :class:`~swiss.fields.Many2one` to
...
``groups``
    ...
    :class:`~swiss.fields.Many2many` to ``res.groups``
